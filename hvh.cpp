...
void HVH::FakeFlick() {
	if (g_input.GetKeyState(g_menu.main.antiaim.fakeflick.get())) {
		g_cl.m_body = -145.f;
		g_cl.m_body = FLT_MAX;
...
void HVH::DoRealAntiAim() {
	...
	if (m_yaw > 0) {
		...
		if (!g_cl.m_lag && g_csgo.m_globals->m_curtime >= g_cl.m_body_pred && (stand || air)) {
			// there will be an lby update on this tick.
				...
				if (stand) {
					...
					case 3:
						...
						// flap.
					case 4:
						negative ? g_cl.m_cmd->m_view_angles.y += 143.f : g_cl.m_cmd->m_view_angles.y -= 100.f;
						negative = !negative;
						break;

						// full/180.
					case 5:
						negative ? g_cl.m_cmd->m_view_angles.y += 180.f : g_cl.m_cmd->m_view_angles.y -= 180.f;
						negative = !negative;
						break;
...
void HVH::DoFakeAntiAim() {
	...
	*g_cl.m_packet = true;
	...
	case 7: {
		...
		negative = !negative;
		g_cl.m_cmd->m_view_angles.y = m_direction + 180.f;
		negative ? g_cl.m_cmd->m_view_angles.y += 90 + std::fmod(g_csgo.m_globals->m_curtime * -105.f, 90.f) : g_cl.m_cmd->m_view_angles.y -= 90 + std::fmod(g_csgo.m_globals->m_curtime * -105.f, 75.f);
		break; }
	...
	case 10: {
		...
		negative = !negative;
		g_cl.m_cmd->m_view_angles.y = m_direction + 180.f;
		negative ? g_cl.m_cmd->m_view_angles.y += 90 + std::fmod(g_csgo.m_globals->m_curtime * -170.f, 132.f) : g_cl.m_cmd->m_view_angles.y -= 90 + std::fmod(g_csgo.m_globals->m_curtime * -170.f, 38.f);
		break; }

		   // desync yaw
	case 11:
		// set base to opposite of direction.
		g_cl.m_cmd->m_view_angles.y = m_direction + 58.f;

		// apply 1 degree jitter.
		g_cl.m_cmd->m_view_angles.y += g_csgo.RandomFloat(1.f, -1.f);
		break;
	...
	case 9: {
		static bool switcher = false;
		g_cl.m_cmd->m_view_angles.y = m_direction + 35.f;
		if (switcher)
			g_cl.m_cmd->m_view_angles.y += 90 + std::fmod(g_csgo.m_globals->m_curtime * -105.f, 20.f);
		else
			g_cl.m_cmd->m_view_angles.y -= 90 + std::fmod(g_csgo.m_globals->m_curtime * -105.f, 50.f);
		switcher = !switcher;
		break; }
		...
		if (g_menu.main.antiaim.lag_enable.get() && !g_csgo.m_gamerules->m_bFreezePeriod() && !(g_cl.m_flags & FL_FROZEN)) {
			...
			for (auto it = activation.begin(); it != activation.end(); it++) {
				...
				if (*it == 5) {
					...
					auto v52 = _mm_shuffle_ps(v73, v73, 170).m128_f32[3] >= 0.15f ? 0.1f : v73.m128_f32[3];
					if (v52 < 0.99f)
						active = true;
				}

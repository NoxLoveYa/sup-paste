...
void AimPlayer::SetupHitboxes(LagRecord* record, bool history) {
	...
	// prefer, stand.
	if (g_menu.main.aimbot.baim1.get(3) && record->m_mode != Resolver::Modes::RESOLVE_NONE && record->m_mode != Resolver::Modes::RESOLVE_WALK && speed < 0.1f)
		m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::PREFER });
	...
	// only, fake.
	if (g_menu.main.aimbot.baim2.get(2) && record->m_mode != Resolver::Modes::RESOLVE_NONE && record->m_mode != Resolver::Modes::RESOLVE_WALK && speed < 0.1f) {
		only = true;
		m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::NORMAL });
	}

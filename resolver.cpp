...
void Resolver::MatchShot(AimPlayer* data, LagRecord* record) {
	...
	else if (delta == 0)
	{
		...
		if (data->m_records.size() >= 2.5f) //was 2.5f then it was 2.0f	then 2.5f again then 1.5f then 3.5f then 2.5f
		{
...
void Resolver::AntiFreestand(LagRecord* record) {
	// constants
	constexpr float STEP{ 4.f }; // was 4 then 5 then 2 then 4 then 10 then 12 then 24 then 1 then 8 then 6 then 180 then 1 then 4
	constexpr float RANGE{ 32.f }; // was 32 then 22 then 40 then 64 then 128 then 180 then 179 then 180 then 32
...
void Resolver::ResolveOverride(Player* player, LagRecord* record, AimPlayer* data) {
	...
	if (data->m_body != data->m_old_body)
	{
		...
		data->m_body_update = record->m_anim_time + 1.10f;
	...
	else
	{
		...
		if (record->m_anim_time >= data->m_body_update) {
			...
			if (data->m_body_index < 1) {
				...
				data->m_body_update = record->m_anim_time + 0.88f; // was 1.10f then 1.25f
...
void Resolver::ResolveWalk(AimPlayer* data, LagRecord* record) {
	...
	// delay body update.
	data->m_body_update = record->m_anim_time + 0.15f;
...
{
	if (player->m_vecVelocity().length_2d() > 0.5f)
...
void Resolver::ResolveStand(AimPlayer* data, LagRecord* record) {
	...
	else if (data->m_body != data->m_old_body && data->m_body_index <= 5) // flick prediction, has .22 flick predict
	{
		...
		data->m_body_update = record->m_anim_time + 1.00f;
		iPlayers[record->m_player->index()] = false;
	...
	else // bruteforce as a last fallback
	{
		...
		{
		case 0:
			record->m_eye_angles.y = away + 90;
			break;

		case 1:
			record->m_eye_angles.y = away - 170;
			break;
...
void Resolver::ResolveAir(AimPlayer* data, LagRecord* record) {
	...
	case 1:
		record->m_eye_angles.y = velyaw - 170.f;
		break;

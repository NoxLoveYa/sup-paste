...
void GUI::think() {
	...
	for (const auto& f : m_forms) {
		...
		if (pressed) {
			Rect header_area = f->GetHeaderRect();
			if (g_input.IsCursorInRect(header_area)) {
				if (f->m_active_element)
					f->m_active_element = nullptr;

				Rect menu = f->GetFormRect();
			...
			if (g_input.IsCursorInRect(f->GetFormRect())) {
				...
				if (g_input.IsCursorInRect(cl)) {
					...
					if (g_input.IsCursorInRect(tabs_area) && !f->m_tabs.empty()) {
						...
						if (f->m_current_header == 1) {
							const auto& t1 = f->m_tabs[0];
							const auto& t2 = f->m_tabs[1];

							Rect tab1{ tabs_area.x + 5, tabs_area.y, tabs_area.w, 16 };

							if (g_input.IsCursorInRect(tab1))
								f->m_active_tab = t1;

							Rect tab2{ tabs_area.x + 5, tabs_area.y + 16, tabs_area.w, 16 };

							if (g_input.IsCursorInRect(tab2))
								f->m_active_tab = t2;
						}
						else if (f->m_current_header == 2) {
							const auto& t1 = f->m_tabs[2];
							const auto& t2 = f->m_tabs[3];
							const auto& t3 = f->m_tabs[8];
							...
							Rect tab3{ tabs_area.x + 5, tabs_area.y + 27, tabs_area.w, 16 };

							if (g_input.IsCursorInRect(tab3))
								f->m_active_tab = t3;
						}
						else if (f->m_current_header == 3) {
							const auto& t1 = f->m_tabs[6];
							const auto& t2 = f->m_tabs[5];
							const auto& t3 = f->m_tabs[4];
						...
						else if (f->m_current_header == 4) {
							const auto& t1 = f->m_tabs[7];

...
void Shots::OnImpact(IGameEvent* evt) {
	...
	if (!trace.m_entity || !trace.m_entity->IsPlayer()){
		//std::string out = tfm::format(XOR("Failed to correct anti-aim\n"));
...
void Shots::OnHurt(IGameEvent* evt) {
	...
	if (g_menu.main.misc.hitmarker.get()) {
		...
		switch (g_menu.main.misc.hitsounds.get() % 100) {
		case 0: break;
	...
	if (g_menu.main.misc.notifications.get(0)) {
		...
		if (hp == 0 && g_menu.main.misc.killsay.get()) {
			...
			case 0:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say ez 1");
				else
					g_csgo.m_engine->ExecuteClientCmd("say get skateboarded on");
				break;

			case 1:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say feel the skateboard sense");
				else
					g_csgo.m_engine->ExecuteClientCmd("say skate or die");
				break;
			...
			case 3:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say your mom fell off her skateboard");
				else
					g_csgo.m_engine->ExecuteClientCmd("say my bad I fucked you up so hard");
				break;

			case 4:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say get resolved");
				else
					g_csgo.m_engine->ExecuteClientCmd("say easiest kill of my life");
				break;
			...
			case 6:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say nice lby breaker lmao");
				else
					g_csgo.m_engine->ExecuteClientCmd("say nice deathrow bro");
				break;

			case 7:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say impact = &destroyed;");
				else
					g_csgo.m_engine->ExecuteClientCmd("say Mode: RESOLVE_LAST_LBY");
				break;

			case 8:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say nice config");
				else
					g_csgo.m_engine->ExecuteClientCmd("say Mode: RESOLVE_FREESTAND");
				break;

			case 9:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say stop holding E bro");
				else
					g_csgo.m_engine->ExecuteClientCmd("say nice ping spike retard");
				break;

			case 10:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say shoot faster");
				else
					g_csgo.m_engine->ExecuteClientCmd("say nice double tap retard");
				break;
			}
		}
	}

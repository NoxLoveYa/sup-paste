...
void Grenades::paint() {
	...
	// render grenade range.
	vec3_t endpos = m_path[m_path.size() - 1];
	if (g_cl.m_weapon->m_iItemDefinitionIndex() == SMOKE || g_cl.m_weapon->m_iItemDefinitionIndex() == HEGRENADE)
		render::world_circle(endpos, 144.f, Color(255, 255, 255, 255));
	else if (g_cl.m_weapon->m_iItemDefinitionIndex() == MOLOTOV || g_cl.m_weapon->m_iItemDefinitionIndex() == FIREBOMB)
		render::world_circle(endpos, 150.f, Color(255, 255, 255, 255));
}
